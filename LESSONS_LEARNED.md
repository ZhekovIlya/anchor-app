...
- **Automated Branch-per-Task Lifecycle (STRICT USER REQUIREMENT):** [DEV] MUST create a new task branch (e.g., `git checkout -b task/description`) and switch to it BEFORE making ANY file changes. NEVER modify files directly on the `main` branch. However, once the task is complete, the agent MUST automatically stage, commit, switch to `main`, `git merge` the task branch, and `git push origin main`. The USER reviews code by accepting IDE terminal executions, so leaving unmerged branches requires tedious manual work. Automate the full lifecycle.
